...
app.post('/api/generate-reqdoc', async (req, res) => {
  ...
  const sysPrompt = `你是一个资深需求分析师。根据需求访谈的全部问答记录，生成一份正式的 **需求分析文档**，供产品评审和开发使用。

此文档遵循「ZFL Reqdoc」方法论，与标准化需求方案文档格式一致，用于：编辑、版本对比、文本协作、评审签字。

## 文档结构（严格按以下 8 个一级章节输出，顺序不可变）
...
1.1 **项目背景** — 描述项目现状和为什么要做这个需求
1.2 **业务目标** — 列出业务层面的目标（省钱、省时、提效、减少错误等）
1.3 **用户目标** — 按角色分别列出每个角色能做什么、获得什么价值
1.4 **本轮要解决的问题** — 逐条列出，每条说明当前痛点
1.5 **不在本轮解决的问题** — 明确排除本次范围的内容
...
2.2 **核心流程**：
- 用自然语言清晰概述主流程的关键节点、分支路径、判断条件、异常路径和结束结果
- 不推荐在 Markdown 中使用 ASCII 流程图（\`|——\` 等字符），因为不同编辑器显示容易乱码
- 如果流程较复杂，用文字分步概述即可
...
按需求范围逐项展开。每条需求至少包含以下 11 个子节：

**3.x.1 适用角色与权限**：列出角色及其在该需求中的权限

**3.x.2 触发入口**（必须区分主入口和辅助入口）：
\`\`\`
**入口 1 — xxx（本期主入口）**：[描述]
**入口 2 — xxx（辅助入口）**：[描述]
\`\`\`
禁止模糊表述（如"本期先做A，B作为P1"）
...
**3.x.4 交互步骤**：1, 2, 3... 逐步说明完整操作过程

**3.x.5 关键状态**：| 状态名 | 说明/展示方式 |

**3.x.6 业务规则**：唯一性约束、即时生效、历史数据兼容等

**3.x.7 数据输入输出**：输入字段（必填/可选）、输出数据格式

**3.x.8 异常或边界情况**：| 异常场景 | 处理方式 |
...
**字段一致性要求**：
- 如在某处新增/修改字段，必须同步检查文档中所有引用该字段的位置
- 不允许同一字段在不同章节描述不一致
...
| 前端开发 | ... | ... |
| 后端开发 | ... | ... |
${requirementType === 'ai' ? '| AI开发 | ... | ... |\n' : ''}| 接口或数据依赖 | ... | ... |
| 状态管理或流程编排 | ... | ... |
| 补充事项 | 操作日志/埋点/消息通知/权限 | ... |

---

### 5. 验收标准

| 验收场景 | 前置条件 | 操作步骤 | 期望结果 | 失败判定 |${requirementType === 'ai' ? ' AI评测集 |' : ''}
|----------|----------|----------|----------|----------|${requirementType === 'ai' ? '----------|' : ''}
| ... | ... | ... | ... | ... |${requirementType === 'ai' ? ' ... |' : ''}
...
- 数据描述务必具体：百分比标注绝对数量、时间标注到年-月-日 时:分、接口超时标注具体秒数
- **"开发必看""重点注意""风险提醒"高亮块要承载真实信息，不写空话或重复正文**
- 输出时要明显突出"研发需要优先查看、重点注意、容易漏掉、阻塞开发、影响联调和验收"的内容`;

  try {
    const messages = [
      { role: 'system', content: sysPrompt },
      { role: 'user', content: `以下是需求访谈的全部问答记录。请严格按照上述 8 章结构生成 reqdoc.md：\n\n${answersText}` },
    ];
