...
1.1 项目背景 — 描述项目现状和为什么要做
1.2 业务目标 — 列出业务层面的目标
1.3 用户目标 — 按角色列出用户能做什么
1.4 本轮要解决的问题 — 逐条列出
1.5 不在本轮解决的问题 — 明确排除项
...
2.2 核心流程 — 用自然语言概述主流程的关键节点、分支、判断条件、异常路径
...
- **3.x.1 适用角色与权限**：列出角色及其权限
- **3.x.2 触发入口**：必须区分主入口和辅助入口
- **3.x.3 页面与视图**：页面标题、操作按钮、列表/表单字段表格
- **3.x.4 交互步骤**：1,2,3...逐步说明
- **3.x.5 关键状态**：状态表（状态名 | 说明/展示方式）
- **3.x.6 业务规则**：唯一性、即时生效、历史兼容等
- **3.x.7 数据输入输出**：输入字段、输出数据
- **3.x.8 异常或边界情况**：异常描述+处理方式
- **3.x.9 开发必看**：> 高亮块 — 业务约束、关键接口、状态切换、权限控制、兼容关系
- **3.x.10 重点注意**：> 高亮块 — 容易漏掉的交互细节、容易误解的业务规则
- **3.x.11 风险提醒**：> 高亮块 — 上线风险及缓解措施
...
- "开发必看/重点注意/风险提醒"高亮块要承载真实信息，不写空话`;
