...
- 如果是 **`0 到 1` 项目**，先遵循 `ui-snow` 中的“参考图审美分析与设计迁移”逻辑：先做参考图、风格、布局、组件语言和交互节奏分析，再结合公司设计规范完成 UI 方案。
...
如果用户还没有确认 `ui.md`，不要抢先声称“设计稿已经完成”或直接进入 Figma 落稿。默认应先把页面结构、交互、状态、视觉方向和组件方案在 `ui.md` 中讲清楚，等待用户确认后，再进行 Figma 设计稿输出。

如果当前 UI 方案属于 PC 管理后台、桌面端工作台、数据平台、运营系统、审批系统、B 端业务系统、CRM 或管理控制台等场景，默认把 [AI_CRB_DESIGN_SPEC.md](/Users/fallin/.codex/skills/ui-snow/references/AI_CRB_DESIGN_SPEC.md) 作为第一设计规范来源；只有在项目现有组件库和明确业务约束要求下，才允许在不破坏整体规范的前提下做局部偏移。
