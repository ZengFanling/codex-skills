...
3. **确认后，补全 `ui.md` 并直接生成可点击交互的 HTML 网页效果稿**
...
- **视觉优先模式（主路径）**：需求拆解 -> 页面骨架 -> `imagegen` 生成 `2~3` 版高保真方向图 -> 选方向 -> refinement -> 回写 `ui.md` -> 规范复核 -> 用户确认 -> HTML 交互网页
- **规范方案模式（复核路径）**：在视觉方向已经比较清楚后，对 `ui.md` / 高保真稿 / HTML 效果稿进行结构、规范、组件一致性、可实现性复核，并补齐约束说明

视觉优先模式不是跳过结构和规范，而是先用图片把审美方向拉起来，再回写成结构化方案；规范方案模式则负责在这个基础上做收口，而不是抢主线。
...
如果当前任务是 **PC 端 UI**，默认先遵循 [AI_CRB_DESIGN_SPEC.md](/Users/fallin/.codex/skills/speckit/references/AI_CRB_DESIGN_SPEC.md) 的桌面端规范，优先考虑：
